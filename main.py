...
CARD_HOLDER = os.environ.get(
    "CARD_HOLDER",
    "نام صاحب کارت",
)

# لینک صفحه شماره کارت / پرداخت روی Render
...
def jalali_date(
    ...
    """تبدیل تاریخ میلادی به شمسی بدون نیاز به jdatetime"""
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [
                ...
                KeyboardButton(
                    text="⚙️ کانفیگ‌ها و آموزش اتصال"
                ),
            ],
        ],
...
def payment_keyboard() -> InlineKeyboardMarkup | None:
    """
    اگر PAYMENT_URL در Render تنظیم شده باشد،
    دکمه بازکردن صفحه پرداخت/شماره کارت را می‌سازد.
    """
...
async def send_welcome(message: Message) -> None:
    ...
    text = (
        ...
        "عزیز، به ربات اینترنت نیم‌بها خوش اومدی.\n\n"
        "با این ربات می‌تونی اشتراک اینترنت نیم‌بها "
        "تهیه کنی و وضعیت حساب و اشتراک‌هات رو ببینی.\n\n"
...
async def show_plans(message: Message) -> None:
    ...
    for plan in PLANS.values():
        lines.append(
            ...
            f"  💰 مبلغ: "
            f"<b>{plan['price']:,} تومان</b>\n"
        )

    lines.append(
        "👇 برای خرید، نام پلن موردنظر را ارسال کنید:\n"
        "<code>پلن ۱ ماهه</code> / "
        "<code>پلن ۲ ماهه</code> / "
        "<code>پلن ۳ ماهه</code>"
    )
...
async def handle_purchase_receipt(
    message: Message,
    state: FSMContext,
) -> None:
    ...
    if not plan:
        ...
        await message.answer(
            "اطلاعات خرید منقضی شده است. "
            "لطفاً مجدداً از منوی "
            "«🛒 خرید اشتراک» اقدام کنید.",
            reply_markup notmain_menu_kb(),
        )
    ...
    await message.answer(
        "✅ <b>رسید شما دریافت شد و برای بررسی "
        "پشتیبانی ارسال گردید.</b>",
        reply_markup=main_menu_kb(),
    )
...
async def start_topup(
    ...
    await message.answer(
        ...
        f"▫️ژ: "
        f"<b>{MIN_TOPUP:,}</b> تومانb> تومان\n"
        f"▫️ حداکثر شارژ: "
        f"<b>{MAX_TOPUP:,}</b> تومان",
        reply_markup=main_menu_kb(),
    )
...
async def topup_amount(
    ...
    try:
        ...
        await message.answer(
            "⚠️ لطفاً مبلغ را فقط به‌صورت عددی "
            "و به تومان وارد کنید.",
            reply_markup=main_menu_kb(),
        )
    ...
    if not MIN_TOPUP <= amount <= MAX_TOPUP:
        await message.answer(
            f"⚠️ مبلغ شارژ باید بین "
            f"{MIN_TOPUP:,} تا {MAX_TOPUP:,} تومان باشد.",
            reply_markup=main_menu_kb(),
        )
    ...
    await message.answer(
        ...
        f"<code>{CARD_NUMBER}</code>\n\n"
        f"👤 به نام: <b>{CARD_HOLDER}</b>\n\n"
        "📸 پس از واریز، لطفاً تصویر فیش "
        "واریزی را ارسال نمایید.",
        reply_markup=payment_keyboard(),
    )
...
async def topup_receipt(
    ...
    await message.answer(
        "✅ <b>رسید شارژ حساب دریافت شد و جهت "
        "تأیید به پشتیبانی ارسال گردید.</b>",
        reply_markup=main_menu_kb(),
    )
...
async def support_info(message: Message) -> None:
    ...
    await message.answer(
        f"{get_header()}"
        "👥 <b>پشتیبانی</b>\n\n"
        "جهت ارتباط مستقیم با پشتیبانی "
        "از آیدی زیر استفاده کنید:\n"
        f"💬 <b>@{username}</b>",
        reply_markup=main_menu_kb(),
    )
...
async def faq(message: Message) -> None:
    text = (
        ...
        "از گزینه «🛒 خرید اشتراک» پلن موردنظر "
        "را انتخاب و پس از واریز، فیش را بفرستید.\n\n"
        "🔹 <b>شارژ حساب چقدر زمان می‌برد؟</b>\n"
        "پس از ارسال فیش، ادمین در اسرع وقت "
        "مبلغ را تأیید و حسابتان را شارژ می‌کند.\n\n"
...
async def configs_guide(message: Message) -> None:
    text = (
        ...
        "پس از فعال‌سازی اشتراک، لینک اتصال "
        "اختصاصی برای شما ارسال خواهد شد."
    )
...
async def run_webhook(
    ...
    webhook_url = (
        f"{base_url.rstrip('/')}{webhook_path}"
    )
    ...
    logging.info(
        "Webhook server started on port %s",
        port,
    )
