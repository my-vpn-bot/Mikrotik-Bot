...
async def cmd_stats(message: types.Message):
    if message.from_user.id == ADMIN_ID:
        ...
        await message.answer(f"📊 **آمار کاربران ربات شانلی**:\n\n👥 تعداد کل کاربران ثبت‌شده: `{count}` نفر")
    else:
...
async def callback_buy_plan(query: types.CallbackQuery):
    ...
    # متن پیش‌فاکتور با تاکید بر پلن انتخابی
    invoice_text = (
        f"🧾 **پیش‌فاکتور نهایی خرید اشتراک**\n\n"
        f"🔹 **نوع اشتراک انتخابی:** `{plan_name}`\n"
        f"💰 **مبلغ قابل پرداخت:** `{plan_price}`\n\n"
        f"💳 **شماره کارت جهت واریز:**\n"
        f"`{CARD_NUMBER}`\n"
        f"👤 **به نام:** {CARD_HOLDER}\n\n"
        f"⚠️ **لطفاً توجه فرمایید:**\n"
        f"برای نهایی کردن خرید، تصویر فیش واریزی خود را به همراه نام پلن انتخاب‌شده (`{plan_name}`) به پشتیبانی ارسال کنید تا اشتراک شما در سریع‌ترین زمان فعال شود."
    )

    await query.message.answer(invoice_text, reply_markup=kb, parse_mode="Markdown")
    await query.answer()
